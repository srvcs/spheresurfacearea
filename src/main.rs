use srvcs_spheresurfacearea::{api::Deps, config::Config, health, router, telemetry};
...
async fn main() {
    ...
    tracing::info!(addr = %cfg.bind_addr, env = %cfg.environment, "srvcs-spheresurfacearea listening");
    let deps = Deps {
        pi_url: cfg.pi_url.clone(),
